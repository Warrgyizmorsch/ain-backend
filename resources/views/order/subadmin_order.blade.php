								...
								<tbody class="allData">
									...
									<tr id="order_{{ $order->id }}" class="{{ ($order->is_read == 1) ? 'bold-row' : '' }}" onclick="markAsRead('{{ $order->id }}')" @if($order->user->feedback_issue == 1) style="color: green;" @endif>										<td>
										...
										<td>
											...
											@endphp
											<div class="mt-1">
												<a href="{{ $writeEmailUrl }}" target="_blank" class="btn btn-icon btn-sm crm-btn-email" style="width: 22px !important; height: 22px !important;" title="Writer Email: {{ $orderRawEmail ?: 'Open Writer Email Channel' }}">
