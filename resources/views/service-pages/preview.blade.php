...
<section class="hero"><div class="wrap hero-grid"><div><div class="crumb">Home › Dynamic Page › {{ $page->subject->name }} Assignment Help</div><div class="rating"><span class="stars">★★★★★</span>Rated 4.9/5 by 25,000+ UK Students</div><h1>{{ $page->hero_heading }}<br><span class="orange">{{ $page->hero_highlight }}</span></h1><p>{{ $page->hero_content }}</p></div><div class="quote"><h2>✨ Get Instant Quote ✨</h2><div class="field">Academic Level &nbsp; — &nbsp; Select Level</div><div class="field">Subject &nbsp; — &nbsp; {{ $page->subject->name }}</div><div class="field">Assignment Type &nbsp; — &nbsp; Select Type</div><div class="field">Deadline &nbsp; — &nbsp; Select Deadline</div><div class="field">Email Address</div><div class="btn">Get Price Now →</div></div></div></section>
<section class="features"><div class="wrap"><h2>Why Students Choose Our {{ $page->subject->name }} Assignment Help?</h2><div class="feature-grid">@foreach(['Qualified Experts','100% Original','On-Time Delivery','24/7 Support','UK-Based Experts','Affordable Pricing'] as $i=>$feature)<div class="feature"><div class="icon">{{ ['♙','✓','◷','☏','⌖','£'][$i] }}</div><h3>{{ $feature }}</h3><p>Professional academic support you can rely on.</p></div>@endforeach</div></div></section>
@if($experts->isNotEmpty())<section><div class="wrap"><h2 class="section-title">Our {{ $page->subject->name }} Assignment Experts</h2><div class="cards">@foreach($experts->take(5) as $expert)<div class="card"><div class="photo">👤</div><div class="card-body"><strong>{{ $expert->name }}</strong><p>{{ $expert->subject ?: $page->subject->name }} Expert</p><small>★ 4.9 · {{ $expert->finish_order ?: 800 }}+ Orders</small></div></div>@endforeach</div></div></section>@endif
