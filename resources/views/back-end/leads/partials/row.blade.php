...
<tr id="lead-{{ $lead->id }}">
    ...
    @endif
        ...
        @php
            ...
            $leadWhatsAppUrl = !empty($rawLeadWAPhone) ? route('whatsapp.chat', ['phone' => $rawLeadWAPhone]) : route('whatsapp.chat');
        @endphp

        {{-- Direct Contact Actions: WhatsApp & Email --}}
        <div class="d-inline-flex align-items-center justify-content-center gap-2 my-1">
            <a href="{{ $leadWhatsAppUrl }}" target="_blank" class="btn btn-icon btn-sm crm-btn-wa" title="WhatsApp: {{ $rawLeadWAPhone ?: 'Open Chat' }}">
