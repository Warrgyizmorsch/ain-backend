...
<div id="kt_content_container" class="container-xxl py-6">
    <div class="card">
        <div class="card-header"><h3 class="card-title">Subject Master</h3></div>
        <div class="card-body">
            @include('layouts.flash')
            <form method="POST" action="{{ route('subjects.store') }}" class="row g-3 mb-8">
                @csrf
                <div class="col-md-9"><label class="form-label required">Subject name</label><input class="form-control" name="name" value="{{ old('name') }}" required></div>
                <div class="col-md-3 d-flex align-items-end"><button class="btn btn-primary w-100">Add Subject</button></div>
            </form>
                ...
                @forelse($subjects as $subject)
                    <tr><form method="POST" action="{{ route('subjects.update', $subject) }}">@csrf @method('PUT')
                        ...
                        <td class="text-end"><button class="btn btn-sm btn-light-primary">Save</button></form>
                            <form class="d-inline" method="POST" action="{{ route('subjects.destroy', $subject) }}" onsubmit="return confirm('Delete this subject?')">@csrf @method('DELETE')<button class="btn btn-sm btn-light-danger">Delete</button></form>
                        </td>
                    </tr>
                @empty <tr><td colspan="4" class="text-center text-muted">No subjects added yet.</td></tr> @endforelse
                </tbody>
            </table></div>
        </div>
    </div>
